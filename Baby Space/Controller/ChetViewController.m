...
- (void)sendAction:(UIButton *)sender {
    ...
    NSString *studentid = [user objectForKey:@"chid"];
    ...
    NSDictionary *parameters = @{@"send_uid":userid,@"receive_uid":_receive_uid,@"content":_inputMess.text,@"send_type":@"0",@"usertype":@"1",@"studentid":studentid};
    NSString *url = @"http://wxt.xiaocool.net/index.php?g=apps&m=message&a=SendChatData";
...
-(void)dealKeyboardFrame:(NSNotification *)changeMess{
    ...
    NSLog(@"%f",self.customTableView.frame.size.height);
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{  //
    ...
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    //    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    NSDictionary *parameters = @{@"send_uid":userid,@"receive_uid":_receive_uid,@"content":_inputMess.text,@"studentid":studentid};
    NSString *url = @"http://wxt.xiaocool.net/index.php?g=apps&m=message&a=SendChatData&send_type=0&usertype=1";
...
//        printf("上传成功");
    ...
    [_inputMess resignFirstResponder];
    [self sendMess:self.inputMess.text]; //发送信息
    [self.customTableView reloadData];
    return YES;
}

...
-(void)setFrameModel:(modelFrame *)frameModel{
    ...
    if (frameModel.myself) {
        ...
        NSURL *a = [NSURL URLWithString: [NSString stringWithFormat:@"http://wxt.xiaocool.net/uploads/microblog/%@", self.selfPhoto]];
    
        [imageView sd_setImageWithURL:a placeholderImage:[UIImage imageNamed:@"girl"]options:SDWebImageRetryFailed];
    }else{
        
        
        NSURL *a = [NSURL URLWithString: [NSString stringWithFormat:@"http://wxt.xiaocool.net/uploads/microblog/%@", self.otherPhoto]];
        
        [imageView sd_setImageWithURL:a placeholderImage:[UIImage imageNamed:@"ic_xihuan"]

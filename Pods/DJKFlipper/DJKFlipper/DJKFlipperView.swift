...
public class DJKFlipperView: UIView {
    ...
    func updateTheActiveView() {
        ...
        if let dataSource = self.dataSource {
            if dataSource.numberOfPages(self) > 0 {
                ...
                let constraintTop = NSLayoutConstraint.constraintsWithVisualFormat("V:|-0-[activeView]-0-|", options: NSLayoutFormatOptions.AlignAllTop, metrics: nil, views: viewDictionary)
                let constraintLeft = NSLayoutConstraint.constraintsWithVisualFormat("H:|-0-[activeView]-0-|", options: NSLayoutFormatOptions.AlignAllLeft, metrics: nil, views: viewDictionary)
    ...
    func getHighestDJKAnimationLayerFromDirection(flipDirection:FlipDirection) -> DJKAnimationLayer? {
        ...
        if animationsInSameDirection.count > 0 {
            animationsInSameDirection.sortInPlace({$0.zPosition > $1.zPosition})
    ...
    func setUpForFlip(animationLayer:DJKAnimationLayer, progress:CGFloat, animated:Bool, clearFlip:Bool) {
        ...
        //_ = DJKFlipperConstants.DurationConstant
